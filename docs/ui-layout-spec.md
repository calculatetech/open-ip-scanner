...
Run a fixture containing conflicting Local, PTR, System, and mDNS names. Confirm the table shows only the preferred plain hostname. Open Details and confirm `Hostname(s):` lists every genuine distinct detected name with its short source labels immediately appended, collapses matching short and FQDN forms without repeating a source, and does not change the table layout or send new traffic.
