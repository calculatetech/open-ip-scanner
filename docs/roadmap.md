...
The audited baseline is `0.2.0` at commit `91e0a7a`; `0.5.2` is the latest manually tested version, versions from `0.5.3` onward are being adversarially reviewed and merged under the explicit waiver for remaining pre-1.0 work, and `0.5.9` is the active APPLICATION-LAYERS scan-session branch. Version 1.0 is not ready. Every unchecked item under “Required for 1.0” is a release gate; the post-1.0 section is explicitly outside the first production release.
...
- [ ] **Split the 3,534-line window into testable layers.** `Medium`
  ...
  - Current progress through `0.5.9`: the build has explicit reusable `ois_core`, `ois_scan`, `ois_linux`, and `ois_ui` targets with narrow public include paths and one-way link dependencies. Target parsing, current OUI normalization/override/lookup policy, settings migration, and target-history persistence live in `ois_core` with direct window-free contracts. A reusable `ScanSession` in `ois_scan` now owns the top-level joinable worker, cancellation token, callback marshaling, overlap rejection, and single completion for both production and hidden-fixture scans; the window consumes typed progress/result/completion signals. Remaining work is to move the production host/probe algorithm behind injectable backends and further reduce window-owned presentation coordination before checking this item complete.
