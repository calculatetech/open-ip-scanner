...
- [ ] **Split the 3,534-line window into testable layers.** `Medium`
  ...
  - Current progress through `0.5.16`: the build has explicit reusable `ois_core`, `ois_scan`, `ois_linux`, and `ois_ui` targets with narrow public include paths and one-way link dependencies. Target parsing, OUI policy, settings migration, and target-history persistence live in `ois_core`. `ScanSession` owns the top-level joinable worker and transfers plain delivery records through a synchronized owner-thread delivery queue. A window-free `ScanEngine` owns bounded scheduling and publication, while `ProductionHostScanBackend` owns per-host discovery and ordered enrichment behind injected probe functions. `LinuxNeighborProbe` owns neighbor execution, `LinuxPingProbe` owns ping attempts, `ServiceProbe` owns service probing, and `HostnameResolver` owns PTR, system, and mDNS coordination outside the window. Remaining work is to move vendor/details helpers out of the window and further reduce presentation coordination before checking this item complete.
