...
The audited baseline is `0.2.0` at commit `91e0a7a`; `0.5.2` is the latest manually tested version, versions from `0.5.3` onward are being adversarially reviewed and merged under the explicit waiver for remaining pre-1.0 work, and `0.5.14` is the active APPLICATION-LAYERS Linux-ping-probe branch. Version 1.0 is not ready. Every unchecked item under “Required for 1.0” is a release gate; the post-1.0 section is explicitly outside the first production release.
...
- [ ] **Split the 3,534-line window into testable layers.** `Medium`
  ...
  - Current progress through `0.5.14`: the build has explicit reusable `ois_core`, `ois_scan`, `ois_linux`, and `ois_ui` targets with narrow public include paths and one-way link dependencies. Target parsing, OUI policy, settings migration, and target-history persistence live in `ois_core`. `ScanSession` owns the top-level joinable worker and transfers plain delivery records through a synchronized owner-thread delivery queue. A window-free `ScanEngine` owns bounded scheduling and publication, while `ProductionHostScanBackend` owns per-host discovery and ordered enrichment behind injected probe functions. `LinuxNeighborProbe` owns neighbor execution and confirmation, and `LinuxPingProbe` now owns interface-scoped ping attempts and cancellation/deadline handling outside the window. Remaining work is to move service sockets, vendor/details, and name-resolution helpers out of the window and further reduce presentation coordination before checking this item complete.
